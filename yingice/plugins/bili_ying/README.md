...
Bilibili 直播订阅、UP 主视频更新订阅与视频信息解析插件。群管理员或 bot 管理员可以为当前群添加直播间订阅或 UP 主视频订阅，插件会定时检查直播间状态与 UP 主最新视频，并在开播、下播或视频更新时向群聊发送提醒。群内发送 Bilibili 视频链接或 BV 号时，插件会自动回复视频名称、BV 号、UP 主与视频封面小图。
...
- 直播订阅必须使用 `@bot bili live ...`，旧的 `@bot bili add/list/remove/delete` 不再作为直播订阅指令。
- UP 主视频订阅添加时会尝试获取并存储 UP 主名称，但不会在添加阶段请求最新视频；后续定时检查成功后会先初始化最新视频基线，再对之后的新视频进行推送。
...
- UP 主视频更新检查使用 WBI 投稿接口 `https://api.bilibili.com/x/space/wbi/arc/search`，插件会通过 `https://api.bilibili.com/x/web-interface/nav` 获取 WBI key 后生成签名参数；同一轮视频订阅批次只获取一次 WBI key，并在该批次内复用。
